...
decode_uri_test() ->
	% rss_wc_lib:decode_uri(<<"http%3A%2F%2Fdigg.com%2Frss%2Ftop.rss">>).
	% rss_wc_lib:decode_uri(<<"http%3A%2F%2Frss.nytimes.com%2Fservices%2Fxml%2Frss%2Fnyt%2FHomePage.xml">>).
...
parse_xml_test() ->
	...
	{ok, Text} = rss_wc_lib:parse_xml(binary_to_list(XMLBody), "//item/description/text()"),
...
parse_text_test() -> 
	...
	{ok, FilteredText} = rss_wc_lib:parse_text(binary_to_list(Text)),
...
tokenize_text_test() ->
	...
	{ok, Tokens} = rss_wc_lib:tokenize_text(binary_to_list(Text), " ="),
...
%	{ok, [FilteredTokens, StopwordsCounts]} = rss_wc_lib:filter_stopwords(Tokens),
%	rss_wc_lib:terminate([], []),
%	?debugFmt("Tokens length: ~b~n", [erlang:length(Tokens)]),
...
count_tokens_test() ->
	...
	{ok, CountedTokens } = rss_wc_lib:count_tokens(Tokens),
%	?debugFmt("Tokens length: ~b~n", [erlang:length(Tokens)]),
...
limit_tokens_test() ->
	...
	Limit = 10,
	{ok, LimitedSortedTokens} = rss_wc_lib:limit_tokens(SortedTokens, Limit),
...
format_to_json_test() -> 
	...
	{ok, JSON_String} = rss_wc_lib:format_to_json(LimitedSortedTokens, StopwordsCount),
